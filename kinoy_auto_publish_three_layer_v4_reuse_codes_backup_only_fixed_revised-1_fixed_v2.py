...
def normalize_social_link_key(val: str) -> str:
    v = safe_text(val).strip()
    if not v:
        return ""
...
def normalize_social_link_url(val: str) -> str:
    v = safe_text(val).strip()
    if not v:
        return ""
    if v.startswith("http://") or v.startswith("https://"):
...
async def add_social_network_db(link: str) -> bool:
    key = normalize_social_link_key(link)
    url = normalize_social_link_url(link)
    if not key or not url:
        return False
    async with aiosqlite.connect(DB_FILE) as db:
        cur = await db.execute(
            "INSERT OR IGNORE INTO social_networks(link_key, link, added_at) VALUES(?, ?, ?)",
            (key, url, now_utc().isoformat()),
        )
        await db.commit()
        return cur.rowcount > 0


async def remove_social_network_db(link: str) -> bool:
    key = normalize_social_link_key(link)
    if not key:
        return False
    async with aiosqlite.connect(DB_FILE) as db:
        cur = await db.execute("DELETE FROM social_networks WHERE link_key = ?", (key,))
        await db.commit()
        return cur.rowcount > 0


async def list_social_networks_db() -> List[Tuple[str, str]]:
    async with aiosqlite.connect(DB_FILE) as db:
        cur = await db.execute("SELECT link, added_at FROM social_networks ORDER BY added_at DESC")
        rows = await cur.fetchall()
    return [(safe_text(link), safe_text(added_at)) for link, added_at in rows]
...
async def get_episode_record_db(series_code: str, episode_number: int) -> Optional[Tuple[Any, ...]]:
    """Qismni yangilashda kerak bo'ladigan barcha maydonlarni qaytaradi."""
    async with aiosqlite.connect(DB_FILE) as db:
        cur = await db.execute(
            """
            SELECT
...
async def auto_assign_series_seasons_from_tmdb(series_code: str, source_title: Optional[str]) -> int:
    title = safe_text(source_title)
    if not title:
        meta = await get_series_meta(series_code)
        if meta:
            title = safe_text(meta[0])

    if not title:
        return 0

    details = await resolve_tmdb("series", title)
    if not details:
        return 0

    available_eps = await get_series_episodes(series_code)
    available_count = len(available_eps)

    raw_seasons = details.get("seasons") or []
    ...
    for season in raw_seasons:
        ...
        # 0-sezon odatda maxsus qism bo'ladi, uni faslga qo'shmaymiz.
        if season_number <= 0:
            continue
        ...
        remaining = available_count - (used_episode - 1)
        # Yetarli qism bo'lmasa, oxirgi faslni yozmaymiz.
        if episode_count > remaining:
            break

        start_ep = used_episode
        end_ep = used_episode + episode_count - 1
        ordered.append((start_ep, end_ep))
        used_episode = end_ep + 1

    if not ordered:
        return 0

    await set_series_seasons_db(series_code, ordered)
    return len(ordered)
...
async def send_media_with_fallback(
    ...
    """Media yuborishda avval file_id, keyin source, so‘ng archive nusxadan foydalanadi."""
    file_id_text = safe_text(file_id)
    file_type_text = safe_text(file_type).lower()

    async def _send_by_file_id() -> bool:
        if not file_id_text:
            return False
        if file_type_text == "video":
            await bot.send_video(user_id, file_id_text, caption=caption, reply_markup=reply_markup)
        elif file_type_text == "document":
            await bot.send_document(user_id, file_id_text, caption=caption, reply_markup=reply_markup)
        elif file_type_text == "animation":
            await bot.send_animation(user_id, file_id_text, caption=caption, reply_markup=reply_markup)
        else:
            await bot.send_document(user_id, file_id_text, caption=caption, reply_markup=reply_markup)
        return True

    async def _copy_from(chat_id: Optional[str], message_id: Optional[int]) -> bool:
    ...
    try:
        if await _send_by_file_id():
            return True
    except Exception:
        logger.exception("send_media_with_fallback: file_id yuborish xatosi")

    try:
        if await _copy_from(source_chat_id, source_message_id):
            return True
    except Exception:
        logger.exception("send_media_with_fallback: source copy xatosi")

    try:
        if await _copy_from(archive_chat_id, archive_message_id):
            return True
    except Exception:
        logger.exception("send_media_with_fallback: archive copy xatosi")
...
async def get_active_backup_channel_ids() -> List[str]:
    rows = await list_managed_channels_db()
    active = []
    for chat_id, title, username, purpose, status, added_at, updated_at in rows:
        if safe_text(purpose) == "storage" and safe_text(status) == "active":
            active.append(str(chat_id))
    return active


async def is_active_backup_storage_chat(chat_id: Any) -> bool:
    """Berilgan chat backup/storage kanali sifatida faol ishlayaptimi."""
    target = str(chat_id)
    async with aiosqlite.connect(DB_FILE) as db:
        cur = await db.execute(
            """
            ...
            WHERE chat_id = ? AND purpose = 'storage' AND status = 'active'
            LIMIT 1
            """,
            (target,),
        )
...
async def build_movie_backup_item(
    code: str,
    title: Optional[str],
    file_id: str,
    file_type: str,
    ...
    return {
        ...
        "file_id": file_id,
        "file_type": file_type,
        "caption": build_backup_caption_for_movie(code, title),
        "title": title,
        "source_chat_id": source_chat_id,
        "source_message_id": source_message_id,
...
async def build_episode_backup_item(
    series_code: str,
...
async def backup_item_to_active_storage_channels(item: Dict[str, Any]) -> bool:
    targets = await get_active_backup_channel_ids()
    if not targets:
        return False
    sent_any = False
...
async def backup_movie_to_active_storage_channels(
    code: str,
    title: Optional[str],
    file_id: str,
    file_type: str,
    ...
    item = await build_movie_backup_item(code, title, file_id, file_type, source_chat_id, source_message_id)
    return await backup_item_to_active_storage_channels(item)


async def backup_nameless_to_active_storage_channels(
    code: str,
    file_id: str,
    file_type: str,
    source_chat_id: Optional[str] = None,
    ...
    item = await build_movie_backup_item(code, None, file_id, file_type, source_chat_id, source_message_id)
    return await backup_item_to_active_storage_channels(item)


async def backup_episode_to_active_storage_channels(
    series_code: str,
    series_title: Optional[str],
    episode_number: int,
    file_id: str,
    ...
    item = await build_episode_backup_item(
        series_code,
        series_title,
        episode_number,
        file_id,
...
async def _export_account_snapshot_to_channels_impl():
    ...
    if not active_targets:
        await safe_send(ADMIN_ID, "⚠️ Avval zaxira kanal qo'shing. 🤖 Bu bot aklount -> ➕ Channel qo'shish -> 💾 Akkountga saqlash")
        return False
...
async def publish_movie_or_series(kind: str, code: str, source_title: Optional[str] = None, force_details: Optional[Dict[str, Any]] = None):
    ...
    if kind == "series":
        ...
        # TMDb'dagi season_count / episode_count asosida fasllarni avtomatik yozamiz.
        await auto_assign_series_seasons_from_tmdb(code, title_uz or source_title)
...
def admin_settings_kb() -> ReplyKeyboardMarkup:
    ...
    rows = [
        ...
        [KeyboardButton(text="📝 Captionlarni tuzatish"), KeyboardButton(text="🛠 Postni tuzatish")],
        ...
        [KeyboardButton(text="📣 Ijtimoiy tarmoq"), KeyboardButton(text="🔗 Set ssilka")],
        [KeyboardButton(text="🧹 Reset ssilka"), KeyboardButton(text=copyright_button)],
        [KeyboardButton(text="👤 Copyright ruxsatlari")],
...
def admin_series_settings_kb() -> ReplyKeyboardMarkup:
    rows = [
        ...
        [KeyboardButton(text="🧩 Fasllarga bo'lish"), KeyboardButton(text="🤖 Auto fasllar")],
        [KeyboardButton(text="↩️ Orqaga"), KeyboardButton(text="❌ Cancel")],
    ]
    return ReplyKeyboardMarkup(keyboard=rows, resize_keyboard=True)


def admin_caption_menu_kb() -> ReplyKeyboardMarkup:
    rows = [
        [KeyboardButton(text="🛠 Post captionini tuzatish")],
        [KeyboardButton(text="🎬 Kino/serial captionini tuzatish")],
        [KeyboardButton(text="↩️ Orqaga"), KeyboardButton(text="❌ Cancel")],
    ]
    return ReplyKeyboardMarkup(keyboard=rows, resize_keyboard=True)


def admin_social_menu_kb() -> ReplyKeyboardMarkup:
    rows = [
        [KeyboardButton(text="➕ Ijtimoiy tarmoq qo'shish")],
        [KeyboardButton(text="➖ Ijtimoiy tarmoq uzish")],
        [KeyboardButton(text="📋 Ijtimoiy tarmoqlar ro'yxati")],
        [KeyboardButton(text="↩️ Orqaga"), KeyboardButton(text="❌ Cancel")],
    ]
    return ReplyKeyboardMarkup(keyboard=rows, resize_keyboard=True)
...
async def groups_inline_kb(missing: List[Tuple[str, Optional[str], Optional[str], Optional[str]]]) -> InlineKeyboardMarkup:
    rows = []
    ...
    social_rows = await list_social_networks_db()
    for link_key, link, added_at in social_rows:
        url = normalize_social_link_url(link)
        if not url:
            continue
        rows.append([InlineKeyboardButton(text="Qo'shilish", url=url)])
...
def build_movie_kb(code: str, title: Optional[str]) -> InlineKeyboardMarkup:
    ...
    rows = [
        [
            ...
            InlineKeyboardButton(text="↗️ Uzatish", callback_data=f"movie:transfer:{code}"),
        ],
...
def build_caption_editor_plan(kind: str) -> List[Tuple[str, str]]:
    if kind == "movie":
        return [
            ("title", "🎬 Film nomi"),
            ("language", "🌐 Tili"),
...
def build_caption_preview_block(caption: Optional[str]) -> str:
    text = safe_text(caption) or "-"
    return truncate_text(text, 3000) or text


def build_admin_movie_caption_from_fields(code: str, data: Dict[str, Any]) -> str:
    return build_movie_caption(
        data.get("title"),
        data.get("quality"),
        data.get("language"),
        data.get("genre"),
...
def build_admin_series_caption_from_fields(code: str, data: Dict[str, Any]) -> str:
    title = safe_text(data.get("title"))
    language = safe_text(data.get("language"))
    genre = safe_text(data.get("genre"))
    country = safe_text(data.get("country"))
...
def get_caption_editor_next_prompt(kind: str, field_key: str) -> str:
    prompts = {
        "title": "Yangi nomni yuboring.",
        "language": "Yangi tilni yuboring.",
        "quality": "Yangi sifatni yuboring.",
        "genre": "Yangi janrni yuboring.",
        "country": "Yangi davlatni yuboring.",
        "year": "Yangi yilni yuboring.",
        "description": "Yangi izohni yuboring.",
    }
    return prompts.get(field_key, "Qiymatni yuboring.")


def build_caption_editor_status_text(kind: str, code: str, current_caption: Optional[str], mode: str) -> str:
    title = "Post captioni" if mode == "post" else "Kino/serial captioni"
    header = f"📝 {title} tuzatish\n\n🔖 Kod: {code}\n\n"
    header += "📄 Hozirgi caption:\n"
    header += build_caption_preview_block(current_caption)
    return header


async def send_admin_caption_editor_step(mode: str, kind: str, code: str, data: Optional[Dict[str, Any]] = None):
    data = data or {}
    plan = build_caption_editor_plan(kind)
    index = int(data.get("index") or 0)
    if index <= 0:
        await safe_send(
            ADMIN_ID,
            build_caption_editor_status_text(kind, code, data.get("current_caption"), mode)
            + f"\n\n{get_caption_editor_next_prompt(kind, plan[0][0])}",
            reply_markup=admin_flow_kb(),
        )
        return
    if index >= len(plan):
        return

    field_key, field_title = plan[index]
    await safe_send(
        ADMIN_ID,
        f"{field_title} yuboring.",
        reply_markup=admin_flow_kb(),
    )


async def finalize_caption_editor(mode: str, kind: str, code: str, data: Dict[str, Any]) -> bool:
    if kind == "movie":
        new_caption = build_admin_movie_caption_from_fields(code, data)
    elif kind == "series":
        new_caption = build_admin_series_caption_from_fields(code, data)
    else:
        return False
    ...
    if post:
        ...
        try:
            ...
            logger.exception("finalize_caption_editor edit failed")
            return False

    if mode == "post":
        await update_publish_post_status(code, caption=new_caption, title=safe_text(data.get("title")) or None)
    else:
        if kind == "movie":
            await update_movie_meta_db(
                code,
                safe_text(data.get("title")) or None,
                safe_text(data.get("year")) or None,
                safe_text(data.get("genre")) or None,
                normalize_quality_input(data.get("quality")),
                safe_text(data.get("language")) or None,
                safe_text(data.get("description")) or None,
                safe_text(data.get("country")) or None,
            )
        else:
            await update_series_meta_db(
                code,
                safe_text(data.get("title")) or None,
                safe_text(data.get("language")) or None,
                safe_text(data.get("description")) or None,
...
async def should_protect_content(chat_id: Any) -> bool:
    chat_int = _safe_int(chat_id)
    if chat_int is not None and chat_int == int(ADMIN_ID):
        return False
    if await is_active_backup_storage_chat(chat_id):
        return False
    if not await copyright_is_enabled():
        return False
    if chat_int is None:
        return True
    return not await is_copyright_allowed(chat_int)
...
async def _send_message_wrapped(chat_id, text, *args, protect_content: Optional[bool] = None, **kwargs):
    if protect_content is None:
        protect_content = await should_protect_content(chat_id)
...
ADMIN_MENU_TEXTS = {
    ...
    "🤖 Bu bot aklount",
    "📣 Publish channel",
    "🚀 Eskilarni post qilish",
    "📝 Captionlarni tuzatish",
    "🛠 Postni tuzatish",
    "📝 Nomni tuzatish",
    "🎞 Nomsizni post qilish",
    "🎬 Kinoni sozlash",
    "📺 Serialni sozlash",
    "📣 Ijtimoiy tarmoq",
    "🔗 Set ssilka",
...
async def handle_admin_text(message: Message):
    ...
    if st and st.get("action") in {"post_caption_editor", "content_caption_editor"}:
        mode = st.get("action").replace("_caption_editor", "")
        step = st.get("step", "wait_code")

        if step == "wait_code":
            code = safe_text(text)
            post = await get_publish_post(code)
            if not post:
                await safe_send(ADMIN_ID, "Bunday post topilmadi.", reply_markup=admin_caption_menu_kb())
                admin_states.pop(ADMIN_ID, None)
                return

            kind = safe_text(post[0])
            if kind not in {"movie", "series"}:
                await safe_send(ADMIN_ID, "Bu post turi hozircha qo'llab-quvvatlanmaydi.", reply_markup=admin_caption_menu_kb())
                admin_states.pop(ADMIN_ID, None)
                return

            current_caption = safe_text(post[10])
            plan = build_caption_editor_plan(kind)
            if not plan:
                await safe_send(ADMIN_ID, "Caption maydonlari topilmadi.", reply_markup=admin_caption_menu_kb())
                admin_states.pop(ADMIN_ID, None)
                return
            ...
            await safe_send(
                ADMIN_ID,
                build_caption_editor_status_text(kind, code, current_caption, mode)
                + f"\n\n{plan[0][1]} yuboring.",
                reply_markup=admin_flow_kb(),
            )
            return
        ...
        if step == "wait_field":
            code = st.get("code")
            kind = st.get("kind")
            plan = st.get("plan") or build_caption_editor_plan(kind)
            index = int(st.get("index") or 0)
            if index < 0 or index >= len(plan):
                admin_states.pop(ADMIN_ID, None)
                await safe_send(ADMIN_ID, "Caption holati buzildi.", reply_markup=admin_caption_menu_kb())
                return

            value = safe_text(text)
            if not value:
                await safe_send(ADMIN_ID, f"{plan[index][1]} bo'sh bo'lmasin. Qayta yuboring.", reply_markup=admin_flow_kb())
                return

            data = st.get("data") or {}
            field_key = plan[index][0]
            data[field_key] = value
            index += 1
            if index >= len(plan):
                ok = await finalize_caption_editor(mode, kind, code, data)
                admin_states.pop(ADMIN_ID, None)
                if ok:
                    await safe_send(
                        ADMIN_ID,
                        f"✅ Caption yangilandi.\nKod: {code}",
                        reply_markup=admin_settings_kb(),
                    )
                else:
                    await safe_send(
                        ADMIN_ID,
                        "Captionni yangilashda xatolik bo'ldi.",
                        reply_markup=admin_settings_kb(),
                    )
                return

            admin_states[ADMIN_ID].update({"index": index, "data": data})
            await safe_send(
                ADMIN_ID,
                f"{plan[index][1]} yuboring.",
                reply_markup=admin_flow_kb(),
            )
            return

    if st and st.get("action") == "auto_season":
        if st.get("step") == "wait_code":
            code = safe_text(text)
            if not re.fullmatch(r"\d+", code):
                await safe_send(ADMIN_ID, "Kod faqat son bo'lsin.", reply_markup=admin_series_settings_kb())
                return
            meta = await get_series_meta(code)
            if not meta:
                await safe_send(ADMIN_ID, "Bunday serial topilmadi.", reply_markup=admin_series_settings_kb())
                admin_states.pop(ADMIN_ID, None)
                return
            try:
                count = await auto_assign_series_seasons_from_tmdb(code, meta[0])
            except Exception:
                logger.exception("auto_season failed")
                count = 0
            finally:
                admin_states.pop(ADMIN_ID, None)
            if count > 0:
                await safe_send(ADMIN_ID, f"✅ Auto fasllar saqlandi. Jami fasl: {count}", reply_markup=admin_series_settings_kb())
            else:
                await safe_send(ADMIN_ID, "TMDb'dan fasl ma'lumoti topilmadi yoki saqlab bo'lmadi.", reply_markup=admin_series_settings_kb())
            return

    if st and st.get("action") in {"social_add", "social_remove"}:
        if st.get("step") == "wait_link":
            link = safe_text(text)
            if not link:
                await safe_send(ADMIN_ID, "Link bo'sh bo'lmasin.", reply_markup=admin_social_menu_kb())
                return
            if st.get("action") == "social_add":
                added = await add_social_network_db(link)
                admin_states.pop(ADMIN_ID, None)
                if added:
                    await safe_send(ADMIN_ID, "✅ Ijtimoiy tarmoq qo'shildi.", reply_markup=admin_social_menu_kb())
                else:
                    await safe_send(ADMIN_ID, "Bu ijtimoiy tarmoq oldinroq qo'shilgan yoki link noto'g'ri.", reply_markup=admin_social_menu_kb())
                return
            removed = await remove_social_network_db(link)
            admin_states.pop(ADMIN_ID, None)
            if removed:
                await safe_send(ADMIN_ID, "✅ Ijtimoiy tarmoq o'chirildi.", reply_markup=admin_social_menu_kb())
            else:
                await safe_send(ADMIN_ID, "Bunday ijtimoiy tarmoq topilmadi.", reply_markup=admin_social_menu_kb())
            return

    if st and st.get("action") == "caption_menu":
        if text == "🛠 Post captionini tuzatish":
            admin_states[ADMIN_ID] = {"action": "post_caption_editor", "step": "wait_code"}
            await safe_send(ADMIN_ID, "Post kodi yuboring.", reply_markup=admin_flow_kb())
            return
        if text == "🎬 Kino/serial captionini tuzatish":
            admin_states[ADMIN_ID] = {"action": "content_caption_editor", "step": "wait_code"}
            await safe_send(ADMIN_ID, "Kino yoki serial kodini yuboring.", reply_markup=admin_flow_kb())
            return
        await safe_send(ADMIN_ID, "Captionlarni tuzatish menyusi.", reply_markup=admin_caption_menu_kb())
        return

    if st and st.get("action") == "social_menu":
        if text == "➕ Ijtimoiy tarmoq qo'shish":
            admin_states[ADMIN_ID] = {"action": "social_add", "step": "wait_link"}
            await safe_send(ADMIN_ID, "Ijtimoiy tarmoq profil havolasini yuboring.", reply_markup=admin_flow_kb())
            return
        if text == "➖ Ijtimoiy tarmoq uzish":
            admin_states[ADMIN_ID] = {"action": "social_remove", "step": "wait_link"}
            await safe_send(ADMIN_ID, "O'chirish uchun ijtimoiy tarmoq havolasini yuboring.", reply_markup=admin_flow_kb())
            return
        if text == "📋 Ijtimoiy tarmoqlar ro'yxati":
            rows = await list_social_networks_db()
            if not rows:
                await safe_send(ADMIN_ID, "Ijtimoiy tarmoqlar ro'yxati bo'sh.", reply_markup=admin_social_menu_kb())
                return
            lines = [f"{i+1}. {link} | {added_at or '-'}" for i, (link, added_at) in enumerate(rows)]
            await safe_send(ADMIN_ID, "Ijtimoiy tarmoqlar ro'yxati:\n\n" + "\n".join(lines), reply_markup=admin_social_menu_kb())
            return
        await safe_send(ADMIN_ID, "Ijtimoiy tarmoq menyusi.", reply_markup=admin_social_menu_kb())
        return
    ...
    if text in {"📝 Captionlarni tuzatish", "📝 Caption"}:
        admin_states[ADMIN_ID] = {"action": "caption_menu", "step": "menu"}
        await safe_send(ADMIN_ID, "Captionlarni tuzatish menyusi.", reply_markup=admin_caption_menu_kb())
        return

    if text == "📣 Ijtimoiy tarmoq":
        admin_states[ADMIN_ID] = {"action": "social_menu", "step": "menu"}
        await safe_send(ADMIN_ID, "Ijtimoiy tarmoq menyusi.", reply_markup=admin_social_menu_kb())
        return
    ...
    if text == "♻️ Serial replace":
        ...
        await safe_send(
            ADMIN_ID,
            "Replace qilish uchun serial kodini yuboring.\n\nMisollar:\n• 1000 — butun serial\n• 1000-8 — faqat 8-qism",
            reply_markup=admin_flow_kb(),
        )
        return
    ...
    if text == "🤖 Auto fasllar":
        admin_states[ADMIN_ID] = {"action": "auto_season", "step": "wait_code"}
        await safe_send(ADMIN_ID, "Auto fasllar uchun serial kodini yuboring:", reply_markup=admin_flow_kb())
        return
    ...
    if text == "🎬 Kino qo'shish":
        if not await get_active_backup_channel_ids():
            await safe_send(ADMIN_ID, "Avval zaxira kanal qo'shing. 🤖 Bu bot aklount -> ➕ Channel qo'shish -> 💾 Akkountga saqlash", reply_markup=admin_account_kb())
            return
...
async def handle_admin_stateful(message: Message):
    ...
    # Agar foydalanuvchi ishchi oqim ichida bo'lsa-yu, boshqa menyuni bossachi,
    # oqimni yopib, bosilgan menyuni oddiy buyruq sifatida qayta ishlaymiz.
    if text in ADMIN_MENU_TEXTS and text not in {"❌ Cancel", "↩️ Orqaga"}:
        admin_states.pop(ADMIN_ID, None)
        await handle_admin_text(message)
        return
    ...
    # Agar admin oqim ichida bo'lsa ham, istalgan asosiy menyu bosilganda
    # joriy state bekor qilinadi va mos bo'lim qayta ochiladi.
    menu_override_texts = {
        "🎬 Kino qo'shish",
        "🎫 Nomsiz kino",
        "📺 Serial qo'shish",
    ...
    if text in menu_override_texts:
        admin_states.pop(ADMIN_ID, None)
        await handle_admin_text(message)
        return
    ...
    if action == "account_save" and step == "confirm":
        ...
        if text == "Ha":
            if not await get_active_backup_channel_ids():
                admin_states.pop(ADMIN_ID, None)
                await safe_send(ADMIN_ID, "Avval zaxira kanal qo'shing. Keyin yana 💾 Akkountga saqlash ni bosing.", reply_markup=admin_account_kb())
                return
            admin_states.pop(ADMIN_ID, None)
    ...
    if action == "add_movie" and step == "wait_quality":
        ...
        await backup_movie_to_active_storage_channels(
            code=code,
            title=data.get("title"),
            file_id=data.get("file_id"),
            file_type=data.get("file_type"),
    ...
    if action == "add_nameless" and step == "wait_media":
        ...
        await backup_nameless_to_active_storage_channels(
            code=code,
            file_id=file_id,
            file_type=file_type,
    ...
    if action == "add_series" and step == "wait_meta":
        ...
        for fid, ftype, src_chat, src_msg in episodes:
            ...
            await backup_episode_to_active_storage_channels(
        ...
        await auto_assign_series_seasons_from_tmdb(series_code, title)
    ...
    if action == "series_replace" and step == "wait_code":
        full_match = re.fullmatch(r"(\d+)(?:-(\d+))?", text)
        if not full_match:
            await safe_send(ADMIN_ID, "Kod noto'g'ri. Faqat 1000 yoki 1000-8 formatida yuboring.", reply_markup=admin_flow_kb())
            return

        series_code = full_match.group(1)
        episode_str = full_match.group(2)
        ...
        if episode_str:
            ...
            episode_row = await get_episode_record_db(series_code, episode_number)
            if not episode_row:
                await safe_send(
                    ADMIN_ID,
                    f"Bunday qism topilmadi. Kod: {series_code}-{episode_number}",
                    reply_markup=admin_main_kb(),
                )
                admin_states.pop(ADMIN_ID, None)
                return
            ...
            await safe_send(
                ADMIN_ID,
                f"{series_code}-{episode_number} topildi. Endi yangi media/video yuboring.",
                reply_markup=admin_flow_kb(),
            )
            return

        await clear_series_episodes_db(series_code)
        admin_states[ADMIN_ID] = {"action": "series_replace", "step": "collect_episodes", "series_code": series_code, "episodes": []}
        await safe_send(ADMIN_ID, "Yangi serial qismlarini yuboring. Tugatgach 'Tugatdim ✅' bosing.", reply_markup=collect_episodes_kb())
        return

    if action == "series_replace_episode" and step == "wait_media":
        file_id = None
        file_type = None
        if message.video:
            file_id = message.video.file_id
        ...
        episode_number = int(st.get("episode_number") or 0)
        if not series_code or episode_number <= 0:
            admin_states.pop(ADMIN_ID, None)
            await safe_send(ADMIN_ID, "Qismni yangilash holati buzildi. Qayta urinib ko'ring.", reply_markup=admin_series_settings_kb())
            return

        episode_row = await get_episode_record_db(series_code, episode_number)
        if not episode_row:
            admin_states.pop(ADMIN_ID, None)
            await safe_send(ADMIN_ID, "Qism ma'lumoti topilmadi.", reply_markup=admin_series_settings_kb())
            return

        old_file_id, old_file_type, episode_title, downloads, source_chat_id, source_message_id, archive_chat_id, archive_message_id = episode_row
        meta = await get_series_meta(series_code)
        series_title = meta[0] if meta else None

        await add_episode_db(
            series_code,
            episode_number,
            file_id,
            file_type,
        ...
        await auto_assign_series_seasons_from_tmdb(series_code, series_title)
        admin_states.pop(ADMIN_ID, None)
        await safe_send(
            ADMIN_ID,
            f"✅ {series_code}-{episode_number} qismi yangilandi.",
            reply_markup=admin_series_settings_kb(),
        )
        return
    ...
    if action == "series_continue" and step == "wait_code":
        ...
        if not meta:
            ...
            admin_states.pop(ADMIN_ID, None)
            return
        admin_states[ADMIN_ID] = {"action": "series_continue", "step": "collect_episodes", "series_code": text, "episodes": [], "title": meta[0]}
    ...
    if action == "series_continue" and step == "collect_episodes":
        if text == "Tugatdim ✅":
            ...
            for fid, ftype, src_chat, src_msg in eps:
                ...
                await add_episode_db(series_code, ep_num, fid, ftype, None, src_chat, src_msg)
            meta = await get_series_meta(series_code)
            await auto_assign_series_seasons_from_tmdb(series_code, meta[0] if meta else None)
            admin_states.pop(ADMIN_ID, None)
...
@dp.callback_query(lambda c: c.data.startswith("movie:transfer:"))
async def cb_movie_transfer(call: CallbackQuery):
    user_id = call.from_user.id
    allowed = await is_copyright_allowed(user_id)
    if allowed:
        await call.answer("✅ Sizda uzatish yoqilgan, bemalol foydalanishingiz mumkin.", show_alert=True)
    else:
        await call.answer(
            "❌ Sizda uzatish funksiyasi yoqilmagan agar yoqmoqchi bolsangiz adminga murojat qiling!",
            show_alert=True,
        )
...
async def cb_play_episode(call: CallbackQuery):
    ...
    ep = await get_episode_db_full(series_code, epn)
    meta = await get_series_meta(series_code)
    ...
    ok = await send_media_with_fallback(
        user_id=call.from_user.id,
        file_id=file_id,
        file_type=file_type,
        caption=caption,
...
async def send_movie_to_user(user_id: int, code: str):
    mv = await get_movie_db_full(code)
    if not mv:
        return False
...
async def send_series_to_user(user_id: int, code: str):
    ...
    details = await resolve_tmdb("series", title)
    if details:
        caption_html, poster_url, title_uz = await build_channel_series_caption(code, details, title)
        try:
            if poster_url:
            ...
            logger.exception("series preview send failed")
            await safe_send(user_id, f"📺 {title}\n\nSerial uchun qismlar pastda.")
    else:
        await safe_send(user_id, f"📺 {title}\n\nSerial uchun qismlar pastda.")
...
async def cmd_start(message: Message):
    ...
    username = safe_text(getattr(message.from_user, "username", None))
    display_name = f"@{username}" if username else safe_text(getattr(message.from_user, "first_name", None)) or "do'stim"
    welcome_text = (
        f"👋 Salom {display_name} botimizga xush kelibsiz!\n\n"
        "🎬 Kinoni ko'rish uchun kino kodini yuboring."
    )
    if ok:
        await update_user_last_validated(message.from_user.id, now_utc())
        await safe_send(message.from_user.id, welcome_text)
    else:
